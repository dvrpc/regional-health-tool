...
export default function IndicatorCard(props: Props) {
    const { indicator, regionPercentile, countyPercentile, value, tractId, county, onSelect, isSelected, collapsed, setCollapsed } = props;
    ...
    const valueParagraph = dataTextMap[indicator][0].replace('%s', valuePctText).replace('%s', 'Census Tract: ' + tractId)
    const regionPctParagraph = dataTextMap[indicator][1].replace('%s', regionPctText).replace('%s', 'Census Tract: ' + tractId).replace('%s', regionPctText)
    ...
    return (
        <div className={`p-2 shadow-md  border-t border-t-dvrpc-gray-7 ${isSelected && 'border-l-4 border-l-dvrpc-blue-3'}  rounded hover:bg-gray-100 hover:cursor-pointer`} onClick={handleClick}>
            ...
            {!collapsed && (
                <div className="flex flex-col gap-2 mt-2 p-2">
                    <p><BoldText text={valueParagraph} /></p>
                    <p><BoldText text={regionPctParagraph} /></p>
                    <p><BoldText text={countyPctParagraph} /></p>
